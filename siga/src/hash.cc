...
int HashLinearProbing::Pesquisa(const char *key)
{
    ...
    // 
    // 2. Verificar se a posição indicada por index é invalida, se for, retorna -1.
    //    Observacao: index é uma posição invalida se o bit flag na posição index for MASK_EMPTY ou MASK_DELETED.
    // 
    ...
    while(true){
        index = (h1 + i) % _table_size;

        if(_hash_table[index]._flag == MASK_EMPTY || _hash_table[index]._flag == MASK_DELETED)
            return -1;

        if(strcmp(_hash_table[index]._hash_key,key) == 0)
            return index;
        else{
            i++;
            if(i == _table_size)
...
int HashLinearProbing::Insere(const char *key, unsigned int position)
{
    ...
    //    Observacao: index é uma posição vazia se o bit flag na posição index for MASK_EMPTY ou MASK_DELETED.
    // 
    // 3. Se for, colocar a chave na posição, 
    // marcar posição como ocupada (MASK_FILLED) 
    // e retornar index;
    //     
    ...
    while(true){
        ...
        if(_hash_table[index]._flag == MASK_EMPTY || _hash_table[index]._flag == MASK_DELETED){
            ...
            _hash_table[index]._position = position;
            _hash_table[index]._flag = MASK_FILLED;
            return index;
        }
        else{
            i++;
